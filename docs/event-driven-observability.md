...
|---|---|---|---|
| Correlation ID | `X-Correlation-Id` | `envelope.CorrelationId` (no custom header) | `CorrelationId` |
| User ID | `X-User-Id` | `X-User-Id` (when present) | `UserId` |
| Session ID | `X-Session-Id` | `X-Session-Id` (when present) | `SessionId` |
