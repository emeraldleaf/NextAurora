...
- **Security + reliability review fixes** (in-flight, uncommitted) — five findings closed in one batch:
...
- **Query handlers materialize tracked entities and map in memory** (CatalogService/Application/Handlers/GetProductByIdHandler.cs:32; OrderService/Features/GetOrderById.cs:29) instead of projecting via `AsNoTracking() + Select(...)`. This trades read perf for repository simplicity since both query and command handlers share the same `GetByIdAsync`. CLAUDE.md "EF Core reads" rule is technically violated. Fix is a `GetByIdProjectedAsync` (or DTO-shaped query method) on each repo + switching the query handlers. Already documented in code comments and `docs/cqrs-data-access.md` as a known cleanup.
- **Payment recovery: gateway-side idempotency keys not yet used.** The current `PaymentRecoveryJob` marks stale Pending payments as `Failed` and lets the buyer retry from a fresh checkout — safe but loses one charge attempt. The proper enhancement is to plumb a stable idempotency key (e.g. `payment.Id.ToString()`) into `IPaymentGateway.ProcessPaymentAsync` and have the recovery path *retry* the gateway with that key. Stripe and most modern gateways accept `Idempotency-Key` headers. Until that lands, the sweeper is mark-Failed-only.
