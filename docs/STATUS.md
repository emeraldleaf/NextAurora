...
7. **Automate the pre-merge concurrency audit in CI** (~30 min). The seven greps are documented in [performance-and-data-correctness.md "Discipline: pre-merge concurrency audit"](performance-and-data-correctness.md#discipline-pre-merge-concurrency-audit) and the current baseline is clean. Wire them into a GitHub Action that fails the PR if any pattern returns a non-empty result outside the documented framework-standard exceptions. Cheap, mechanical, catches regressions the line-level reviewer might miss.
