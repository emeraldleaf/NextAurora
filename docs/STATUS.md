...
- **Query handlers materialize tracked entities and map in memory** ([CatalogService/Application/Handlers/GetProductByIdHandler.cs:32](../CatalogService/CatalogService.Application/Handlers/GetProductByIdHandler.cs#L32); [OrderService/Features/GetOrderById.cs:29](../OrderService/Features/GetOrderById.cs#L29); [PaymentService/Features/GetPaymentById.cs](../PaymentService/Features/GetPaymentById.cs); [ShippingService/Features/GetShipmentByOrder.cs](../ShippingService/Features/GetShipmentByOrder.cs)) instead of projecting via `AsNoTracking() + Select(...)`. This trades read perf for repository simplicity since both query and command handlers share the same `GetByIdAsync`. CLAUDE.md "EF Core reads" rule is technically violated. Fix is a `GetByIdProjectedAsync` (or DTO-shaped query method) on each repo + switching the query handlers. Already documented in code comments and [docs/cqrs-data-access.md](cqrs-data-access.md) as a known cleanup. **This was code-review finding #5; the EF Core audit (2026-05-22) re-confirmed it's the highest-value next polish.**
