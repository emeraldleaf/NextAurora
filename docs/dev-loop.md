...
| **MCP servers** | Canon in `CLAUDE.md` + paired `docs/*.md` + every-session auto-load + greppable surfaces already gives Claude Code persistent decision context. An MCP server adds an external tool layer on top of decisions that already live in the repo where the code does — convergence in the wrong direction. Tools live near the decisions, not vice versa. See [the 2026-06-07 Miro-MCP audit](../.claude/audits/INDEX.md) for the worked comparison (canon-in-repo vs Miro+MCP). |
