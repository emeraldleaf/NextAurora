...
- **`CatalogService.Application` references `Microsoft.EntityFrameworkCore`.** That's the Clean Architecture trade — Application can use EF directly for query/command logic, but the Domain layer stays EF-free. Same call shape as the VSA services; the layer rule still holds.
- **Exception: read-store ports in Clean Architecture.** `IProductReadStore` (CatalogService) is the one read-side port that survives — its existence is a layer-rule consequence (Domain can't reference Contracts where DTOs live), not a wrapper-around-EF pattern. The implementation in `Infrastructure/Repositories/ProductReadStore.cs` is what touches `DbContext`. For VSA services there's no such layer constraint, so reads happen directly in the handler.
