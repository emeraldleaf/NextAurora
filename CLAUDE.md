...
Correlation/user/session context is propagated across HTTP + Service Bus by `CorrelationIdMiddleware` (HTTP entry), `ContextPropagationMiddleware` (Wolverine incoming), and `OutgoingContextMiddleware` (outbound). Wired via `opts.AddNextAuroraContextPropagation()` in each service's `Program.cs`. **Full mechanism + headers/baggage mapping + scope behavior + wiring details: see [docs/architecture.md "Observability & Context Propagation"](docs/architecture.md#cross-cutting-concerns).**
