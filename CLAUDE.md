...
- **In-handler transactional publishing needs a method-injected `IMessageContext`, NOT the constructor-injected `IEventPublisher`.** Wolverine only enlists in the handler's outbox transaction the `IMessageContext` it injects as a **`HandleAsync` method parameter**. A *constructor*-injected `IMessageBus`/`IMessageContext` (what the `IEventPublisher` shim wraps) is **not** enlisted — under Wolverine 6 a publish through it fires *immediately*, before the handler commits. This silently broke PaymentService's Acceptor→Gateway split on the 5→6 upgrade: the local `PaymentProcessingRequested` continuation reached the Gateway handler before `Payment(Pending)` committed, the Gateway found no row, and the payment stuck in Pending. **Rule:** when a handler publishes a message that must be staged in *its* transaction — especially a local continuation a downstream handler reads back — take `IMessageContext` as a `HandleAsync` parameter and publish through it; `IEventPublisher` stays fine for fire-and-forget external events. Reference: [PaymentService/Features/ProcessPayment.cs](PaymentService/Features/ProcessPayment.cs) `ProcessPaymentHandler`. Full Wolverine 5→6 upgrade notes (RuntimeCompilation package split, `ServiceLocationPolicy` default flip, this enlistment trap): [docs/project-decisions.md "Wolverine 5→6 upgrade"](docs/project-decisions.md).
