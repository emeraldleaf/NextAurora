...
NextAurora is a .NET 10 microservices e-commerce platform using Aspire, RabbitMQ (via Wolverine), gRPC, EF Core, and a React SPA storefront. It follows DDD, CQRS, and event-driven architecture.
...
- **JWT validation (explicit, not implicit)**: `TokenValidationParameters` must explicitly set:
  ...
  - `ClockSkew = TimeSpan.FromSeconds(30)` — default is **5 minutes**, which on the realm's 5-minute access tokens would double every token's effective lifetime.
  ...
  - **`RequireHttpsMetadata` is fail-closed outside Development** — never derived silently from the authority's scheme. An http authority in Production must fail loudly at startup (plaintext OIDC/JWKS fetch = MITM can inject signing keys); legitimate internal-http deployments opt out explicitly via `Authentication:RequireHttpsMetadata=false`, which logs a warning.
- **Keycloak token policy is pinned explicitly in `nextaurora-realm.json`** — never rely on realm defaults: 5-minute access tokens (`accessTokenLifespan: 300`), single-use rotated refresh tokens (`revokeRefreshToken: true` + `refreshTokenMaxReuse: 0`), session idle 30m / max 10h. If the access-token lifespan changes, update the ClockSkew rationale with it. Rationale table: [docs/project-decisions.md §7](docs/project-decisions.md). Realm edits need a local Keycloak re-import to take effect.
...
- **Async events** (RabbitMQ via Wolverine): For workflow orchestration (order -> payment -> shipping -> notification)
