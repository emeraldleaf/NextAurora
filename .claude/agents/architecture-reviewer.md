...
- **Outbox-outside-handler atomicity (CRITICAL).** A sweeper that publishes events must use Wolverine's non-handler outbox: `outbox.Enroll(context)` → entity work → `outbox.PublishAsync(...)` → `await outbox.SaveChangesAndFlushMessagesAsync(ct)`, which commits the entity write and the staged envelope together. If instead it hand-rolls an EF transaction, the wrapper MUST call `await context.SaveChangesAsync(ct)` AFTER the publish and BEFORE `tx.CommitAsync(ct)` — without it Wolverine's staged envelope never reaches `wolverine.outgoing_envelopes` and the event is silently dropped. Reference: `PaymentRecoveryJob.RecoverOneAsync`. Proven by `PaymentRecoveryAtomicityTests`: force the commit to fail after the publish and assert nothing dispatched.
