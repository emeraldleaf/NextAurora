...
| 2026-06-08 | [VSA — When it goes wrong vs when it works](2026-06-08-vsa-when-it-works.md) (Anton Martyniuk infographic) | ⚙️ Most rules aligned (no MediatR, no IHandler interface, simple handler class, no business logic in endpoint). Two material divergences: **file granularity** (NextAurora: 1 file per slice with command+validator+handler co-located; Anton: 4 files per slice) and **expected-error handling** (NextAurora throws exceptions; Anton recommends `Result<T>`) | No action — three optional consolidations flagged: line-count soft cap on feature files (Anton's ~500 line warning), exception-vs-Result stance not encoded, no-IHandler-interface rationale not codified |
